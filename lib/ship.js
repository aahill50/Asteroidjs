(function () {
  ...
  Ship.prototype.draw = function (ctx) {
    ...
    var shipR = [PoC(r*.9, deg - 225)[0], PoC(r*.9, deg - 225)[1]];
		...
		var thrusterL = Asteroids.Util.findPointOnCircle(shipBack, ((Math.random() + 0.5) * this.thrusterLevel) * this.radius, this.degFacing + 160);
		var thrusterR = Asteroids.Util.findPointOnCircle(shipBack, ((Math.random() + 0.5) * this.thrusterLevel) * this.radius, this.degFacing + 200);
  ...
  Ship.prototype.power = function (impulse, dir) {
		if (this.thrusterLevel < 10) {
			this.thrusterLevel += 0.4;
		}
	...
	Ship.prototype.updateThruster = function () {
		this.thrusterLevel > 0.1 ? this.thrusterLevel -= 0.1 : this.thrusterLevel = 0;
	};
